...
class SubNetwork(nn.Module):
    def __init__(self, input_chns, output_chns, chns):
        ...
        self.conv6 = nn.Conv2d(in_channels=chns, out_channels=output_chns, kernel_size=3, stride=1, padding=1, bias=True)
        self.conv7 = nn.Conv2d(in_channels=output_chns, out_channels=chns, kernel_size=1, stride=1, padding=0, bias=True)
        self.conv8 = nn.Conv2d(in_channels=chns, out_channels=output_chns, kernel_size=1, stride=1, padding=0, bias=True)
        self.relu = nn.ReLU(inplace=True)

    def forward(self, x, pre_feature):
        out1 = self.relu(self.conv1(x))
        out2 = self.relu(self.conv2(out1))
        feature = self.relu(self.conv3(out2)) + pre_feature
        out = self.relu(self.conv4(feature)) + out2
        out = self.relu(self.conv5(out)) + out1
        out = self.relu(self.conv6(out)) + self.conv8(self.relu(self.conv7(x)))
        return out, feature
